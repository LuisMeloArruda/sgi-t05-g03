...
function onPointerMove( event ) {
	...
	if (document.pointerLockElement === this.domElement) {
		this._moveState.pitchDown = event.movementY / 20
		this._moveState.yawRight = event.movementX / 20

		this._updateRotationVector()

		const current_z = this._rotationVector.z

		requestAnimationFrame(() => {
			this._rotationVector.set(0, 0, current_z)
		});
	  }
}
